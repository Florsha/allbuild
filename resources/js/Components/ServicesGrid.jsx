...
export default function ServicesGrid({ services, showSelect = false, onSelect }) {
  return (
      ...
      {services.map((service) => (
        ...
        >
          ...
          <div className="relative z-10 p-8 text-center flex flex-col items-center h-full space-y-4">
            {iconMap[service.title] ?? <PuzzlePieceIcon className="w-12 h-12 text-yellow-400" />}
            ...
            {showSelect && (
              <button
                ...
                className="mt-4 bg-yellow-400 px-6 py-2 rounded-full font-semibold"
              >
                Select
              </button>
            )}
